...
public final class NumEncoder extends Encoder {
	...
	public final byte[] encode(final Index index, final ISOFormat format, final String data) {
		...
		if(data.length() > maxlen) throw new RuntimeException(name()+" Field no "+index.fIndex+" with input data length "+data.length()+" is  larger than configured length "+maxlen);
	...
	public final String decode(final Index index, final ISOFormat format, final byte[] bytes) {
		final int maxlen = format.length[index.fIndex];
		if(index.bIndex + maxlen > bytes.length) throw new RuntimeException(name()+" Field no "+index.fIndex+" of length "+maxlen+" exceeds available data with byte index "+index.bIndex+" data "+ByteHexUtil.byteToHex(bytes));
